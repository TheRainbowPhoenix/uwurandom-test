...
static int uwu_write_chars(uwu_state* state, char* buf, size_t n) {
    ...
    while (total_written < n) {
        ...
        // NOTE FROM LDA: chars_written used to be a size_t, which is unsigned, thus causing the next condition 
        // to always be ignored/optimised out.
        // Bit silly, I know.
        int chars_written = uwu_exec_op(state, buf + total_written, n - total_written);
